...
setup_brush(BRUSH * out_brush, BRUSH * in_brush)
{
	...
	if (out_brush->style & 0x80)
	{
        ...
        brush_data = NULL; //cache_get_brush_data(colour_code, cache_idx);
...
process_bmpcache2(STREAM s, uint16 flags, RD_BOOL compressed)
{
    ...
    bitmap = NULL; //ui_create_bitmap(width, height, bmpdata);
...
process_colcache(STREAM s)
{
    ...
    hmap = NULL; //ui_create_colourmap(&map);
...
process_fontcache(STREAM s)
{
	...
	for (i = 0; i < nglyphs; i++)
	{
        ...
        bitmap = NULL; //ui_create_glyph(width, height, data);
