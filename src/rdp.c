		    ...
		    char *password, char *program, char *directory)
{
	...
	int len_domain = 2 * (int)strlen(domain);
	int len_user = 2 * (int)strlen(user);
	int len_password = 2 * (int)strlen(password);
	int len_program = 2 * (int)strlen(program);
	int len_directory = 2 * (int)strlen(directory);

	/* length of strings in TS_EXTENDED_PACKET includes null terminator */
	int len_ip = 2 * (int)strlen(ipaddr) + 2;
	int len_dll = 2 * (int)strlen("C:\\WINNT\\System32\\mstscax.dll") + 2;
...
rdp_enum_bmpcache2(void)
{
	...
	offset = 0;
    num_keys = 0; //pstcache_enumerate(2, keylist);
