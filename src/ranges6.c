...
int
my_aton6(const char *host, unsigned char *addr, size_t sizeof_addr)
{
    struct Range6 range;
    unsigned offset = 0;
    ...
    range = range6_parse(host, &offset, max);
    if (range.begin.hi != range.end.hi || range.begin.lo != range.end.lo)
        return 0;
    if (host[offset] != '\0')
        return 0;

    addr[ 0] = (unsigned char)(range.begin.hi>>56ULL);
    addr[ 1] = (unsigned char)(range.begin.hi>>48ULL);
    addr[ 2] = (unsigned char)(range.begin.hi>>40ULL);
    addr[ 3] = (unsigned char)(range.begin.hi>>32ULL);
    addr[ 4] = (unsigned char)(range.begin.hi>>24ULL);
    addr[ 5] = (unsigned char)(range.begin.hi>>16ULL);
    addr[ 6] = (unsigned char)(range.begin.hi>> 8ULL);
    addr[ 7] = (unsigned char)(range.begin.hi>> 0ULL);
    addr[ 8] = (unsigned char)(range.begin.lo>>56ULL);
    addr[ 9] = (unsigned char)(range.begin.lo>>48ULL);
    addr[10] = (unsigned char)(range.begin.lo>>40ULL);
    addr[11] = (unsigned char)(range.begin.lo>>32ULL);
    addr[12] = (unsigned char)(range.begin.lo>>24ULL);
    addr[13] = (unsigned char)(range.begin.lo>>16ULL);
    addr[14] = (unsigned char)(range.begin.lo>> 8ULL);
    addr[15] = (unsigned char)(range.begin.lo>> 0ULL);
